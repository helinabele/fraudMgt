...
export class FraudInvestigationReportComponent implements OnInit {
  ...
  trackId = (_index: number, item: IFraudInvestigationReport): string =>
    this.fraudInvestigationReportService.getFraudInvestigationReportIdentifier(item);
  ...
  delete(fraudInvestigationReport: IFraudInvestigationReport): void {
    const modalRef = this.modalService.open(FraudInvestigationReportDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.fraudInvestigationReport = fraudInvestigationReport;
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.fraudInvestigationReports = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IFraudInvestigationReport[] | null): IFraudInvestigationReport[] {
    return data ?? [];
  }
  ...
  protected queryBackend(page?: number, predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.fraudInvestigationReportService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
